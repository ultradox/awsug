      ...
      <v-card>
        <v-card-text>
          ...
          <v-card-actions v-if="!distractionFree">
            ...
            <v-btn color="warning" class="px-4" @click="loadBlog()">
              Reset <v-icon right dark>mdi-undo-variant</v-icon>
            </v-btn>
            <v-spacer></v-spacer>
            <span>
...
export default {
  ...
  methods: {
    ...
    loadBlog() {
      this.form.anchor = this.blog.anchor;
      this.form.author = this.blog.author;
      this.form.socLink = this.blog.socLink;
      this.form.title = this.blog.title;
      this.form.summary = this.blog.summary;
      this.form.content = this.blog.content;
      this.info = "Your content has been reloaded 🤓";
      this.snackbar = true;
    },
    async submit() {
      const input = {
